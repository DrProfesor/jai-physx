...
uint8_t px_controller_move(Px_Controller controller_handle, Px_Vector3f32 displacement, float dt, uint32_t mask_index) {
	PxController* controller = (PxController*) controller_handle;
	...
	return (uint8_t) controller->move(disp, 0.001f, dt, filters);
}
